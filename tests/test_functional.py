...
async def test_markitwire_context_awareness():
    ...
    try:
        request = SupportRequest(
            ...
            issue_description="my trade has book2 resolved to 'MarkitWire' but it did not feed outbound",
            lob="platform"
        )
        ...
        assert result is not None, "System should provide recommendations for MarkitWire issues"
        ...
        assert "verify book2" not in recommendations.lower(), "Should skip book2 verification when user stated it's resolved"
        
        # Check that block events code is provided
        assert "ds.evInfo()" in recommendations, "Should provide block events implementation code"
...
async def test_vague_query_handling():
    ...
    try:
        ...
        assert result is None, "System should stay silent for vague queries with no relevant knowledge"
    ...
    for pattern in hardcoded_patterns:
        for py_file in python_files:
            ...
            if "markitwire" in pattern.lower() and "markitwire" in content.lower():
                ...
                lines = content.split('\n')
                for line in lines:
                    if 'if' in line.lower() and 'markitwire' in line.lower() and '==' in line:
                        pytest.fail(f"Found hardcoded pattern in {py_file}: {line.strip()}")
