...
class VectorKnowledgeBase:
        ...
        if EMBEDDINGS_AVAILABLE:
            try:
                if self.use_openai:
                    ...
                    print(
                        "Using OpenAI text-embedding-3-small for superior semantic search"
                    )
                else:
    ...
    def _generate_embedding(self, text: str):
        ...
        if self.use_openai and isinstance(self.embeddings_model, str):
            ...
            response = client.embeddings.create(model=self.embeddings_model, input=text)
  ...
  "reasoning": "explanation of the user's actual problem"
    ...
    def parse_markdown_sections(
        ...
        for line in lines:
            ...
            if header_match:
                ...
                # Create new section
                current_path = (
                    section_stack[-1]["path"][:] if section_stack else [doc_name]
                )
        ...
        if current_section["content"]:
            ...
            if content_text:
                sections.append(
                    (full_title, content_text, "/".join(current_section["path"]))
                )
        ...
        try:
            ...
            for file_path in RESOURCES_DIR.glob("*.md"):
                ...
                for section_title, section_content, section_path in sections:
                    ...
                    if (
                        clean_section_title == doc_name
                    ):  # If it's just the document name, use content only
                        text_to_embed = section_content[:1000]
                    else:
                        text_to_embed = (
                            f"{clean_section_title} {section_content[:1000]}"
                        )
    ...
    async def semantic_search(self, query: str, top_k: int = 5) -> List[Dict]:
        ...
        try:
            ...
            similarities = cosine_similarity(query_embedding, self.section_embeddings)[
                0
            ]
            ...
            for idx in top_indices:
                ...
                if similarity_score > 0.2:
                    ...
                    section_text = (
                        f"{section['section_title']} {section['content']}".lower()
                    )
                    ...
                    for concept in key_concepts:
                        if concept.lower() in section_text:
                            concept_adjustment += (
                                0.05  # Small boost per matching key concept
                            )
...
async def search_knowledge(
    ...
    print(
        "ERROR: Vector embeddings not available - system requires vector search capabilities"
    )
    return []
...
def prepare_analysis_prompt(
    query: str, knowledge_content: str, affected_system: Optional[str] = None
) -> str:
...
3. Include all troubleshooting steps from the knowledge base, whether they have code examples or not
...
Based on the user's specific issue and the knowledge base content, provide a direct list of troubleshooting steps. Use a simple numbered format without additional sections or headers:

1. [First troubleshooting step using ONLY information from the knowledge base]
2. [Second step with exact commands/code from the knowledge base]  
3. [Continue with specific steps, skipping any the user has already confirmed]
4. [Include escalation guidance at the end if provided in the knowledge base]

CRITICAL: Include ALL troubleshooting steps mentioned in the knowledge base content, even if they don't have code examples. Steps like "check eligibility object's IneligibilityReasons" should be included as written.

PRIORITY ORDER: Use content from the main knowledge base sections first, then incorporate any additional implementation details from follow-up searches. Ensure no troubleshooting steps are omitted.

Important: Provide steps in a simple numbered list format. Do not create multiple sections, headers, or subsections. Keep it as a straightforward troubleshooting checklist.
...
async def main():
    ...
    parser.add_argument(
        "--port", type=int, default=8002, help="Port for SSE connections"
    )
