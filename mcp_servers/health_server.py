...
def _initialize_mock_data():
    ...
    services = [
        "trade-booking-service", "risk-engine", "markitwire-gateway", 
        "data-pipeline", "reconciliation-service", "trade-database"
    ]
...
@mcp.tool()
def query_service_logs(service_name: str, hours_back: int = 1, max_lines: int = 100) -> List[Dict]:
    ...
    for entry in LOG_ENTRIES:
        if (entry["service_name"] == service_name and 
...
async def main():
    ...
    parser.add_argument("--port", type=int, default=8003, help="Port for SSE connections")
