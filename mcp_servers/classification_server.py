...
def _build_classification_prompt(user_request: str, team_config: TeamConfigManager, team: str) -> str:
...
class ClassificationServer:
        ...
        async def handle_call_tool(name: str, arguments: Dict[str, Any]) -> List[TextContent]:
            ...
            if name == "classify_support_request":
                ...
                try:
                    ...
                    return [TextContent(type="text", text=json.dumps(team_info, indent=2))]
                ...
                try:
                    ...
                    if category not in categories:
                        error_result = {"error": f"Category '{category}' not found for team '{team}'"}
                        return [TextContent(type="text", text=json.dumps(error_result, indent=2))]
                    
                    return [TextContent(type="text", text=json.dumps(categories[category], indent=2))]
                except ValueError as e:
                    error_result = {"error": str(e)}
                    return [TextContent(type="text", text=json.dumps(error_result, indent=2))]
    ...
    def _parse_llm_response(self, llm_response: str, user_request: str, team: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if category in team_categories:
                classification["suggested_workflow"] = team_categories[category].get("workflow", "technical_guidance")
            else:
            ...
            result = {
                ...
                "reasoning": classification.get("reasoning", "Classification based on LLM analysis"),
                "team": team
            }
    ...
    async def _classify_support_request(self, user_request: str, team: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Sending sampling request for classification: {user_request[:100]}...")
            response = await self.session.send_request(sampling_request, CreateMessageResult)
            ...
            if category in CATEGORIES:
                classification["suggested_workflow"] = CATEGORIES[category].get("workflow", "technical_guidance")
            else:
                classification["suggested_workflow"] = "technical_guidance"
...
async def main():
    ...
    parser.add_argument("--port", type=int, default=8001, help="Port for SSE connections")
    ...
    if args.connection == "stdio":
        ...
        async with stdio_server() as (read_stream, write_stream):
            ...
            classification_server.session = ServerSession(read_stream, write_stream, init_options)
            
            await classification_server.server.run(read_stream, write_stream, init_options)
