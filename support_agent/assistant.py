...
from .models import AnalysisResult, Classification, SupportRequest
...
class SupportAssistant:
    ...
    def __init__(self, config: Optional[MCPConfig] = None):
        ...
        self._llm_api_key = os.getenv("OPENAI_API_KEY") or os.getenv(
            "ANTHROPIC_API_KEY"
        )
        self._llm_api_base = self.config.llm.api_base or os.getenv(
    ...
    async def start_mcp_servers(self) -> bool:
        ...
        try:
            if self.config.connection_method == ConnectionMethod.STDIO:
                ...
                print(
                    f"❌ Unsupported connection method: {self.config.connection_method}"
                )
                return False
    ...
    async def _start_stdio_servers(self) -> bool:
        ...
        try:
            for name, server_config in self.config.servers.items():
                ...
                read, write = await self._context_stack.enter_async_context(
                    stdio_context
                )
        ...
        try:
            ...
            enhanced_knowledge_data = (
                await self._enhance_knowledge_with_recursive_searches(
                    ...
                    depth=self.config.knowledge_search_depth,
                )
            )
            ...
            return {
                ...
                "analysis": AnalysisResult(
                    ...
                    knowledge_results=(
                        knowledge_data if isinstance(knowledge_data, list) else []
                    ),
    ...
    async def _enhance_knowledge_with_recursive_searches(
        self, knowledge_data, user_request: str, depth: int = 1
    ):
        ...
        for current_depth in range(depth):
            ...
            for item in current_level_data:
                ...
                gap_concepts = await self._identify_knowledge_gaps(
                    content, current_depth, user_request
                )
            ...
            for gap_concept in gaps_to_search[
                :2
            ]:  # Limit to 2 follow-up searches per level to avoid token explosion
                try:
                    self._update_tool_call_status(
                        f"Knowledge(L{current_depth+1}: {gap_concept[:30]}...)"
                    )
                    ...
                    if (
                        not follow_up_results.content
                        or len(follow_up_results.content) == 0
                    ):
                    ...
                    if isinstance(follow_up_data, list):
                        for item in follow_up_data:
                            if (
                                ...
                                and item.get("relevance_score", 0) > 0.1
                            ):
    ...
    async def _identify_knowledge_gaps(
        self, content: str, current_depth: int, user_request: str = ""
    ) -> list:
...
Return ONLY short, searchable phrases (2-4 words) that would find implementation code. Examples:
- "check block events" (not "check if there are any block events in the system")
- "validate trade" (not "validate the trade in the destination system")
- "check logs" (not "check logs to verify message processing")
...
Focus on finding actionable next steps that are mentioned but not implemented with code."""
        ...
        try:
            ...
            sampling_request = CreateMessageRequestParams(
                ...
                temperature=0.0,  # Force deterministic gap identification
            )
    ...
    async def _should_assistant_handle_request(
        self, classification: Classification, issue_description: str
    ) -> bool:
...
Answer with just "HANDLE" or "HUMAN_REVIEW"."""
    ...
    async def _generate_recommendations(
        ...
        if isinstance(knowledge_data, list) and len(knowledge_data) > 0:
            ...
            primary_results = [
                item for item in knowledge_data if not item.get("is_followup", False)
            ]
            followup_results = [
                item for item in knowledge_data if item.get("is_followup", False)
            ]

            # Combine top 2-3 primary results for comprehensive guidance
            if primary_results:
                # Sort by relevance score and filter by quality threshold
                sorted_results = sorted(
                    primary_results,
                    key=lambda x: x.get("relevance_score", 0),
                    reverse=True,
                )
                # Only include high-relevance results (60%+ relevance) and limit to top 3
                high_quality_results = [
                    result for result in sorted_results
                    if result.get("relevance_score", 0) >= 0.6
                ]
                top_results = high_quality_results[:3]  # Focus on most relevant sections
                
                if top_results:
                    best_result = top_results[0]  # Primary source for metadata
                else:
                    # Fallback: if no high-quality results, use top result but flag low confidence
            ...
            # Only include follow-up results that are reasonable matches (30%+ relevance)
            # Follow-ups search for implementation details which may have lower semantic similarity
            # but still provide meaningful additional value
            quality_followups = [
                followup for followup in followup_results 
                if followup.get("relevance_score", 0) >= 0.3
            ]
            
            if quality_followups:
                combined_content += "\n\n---\nADDITIONAL KNOWLEDGE FOUND:\n"
                for followup in quality_followups:
            ...
            try:
                ...
                # Build comprehensive source attribution showing all sources actually used
                # Separate primary and follow-up sources that were actually included
                primary_sources_used = [
                    source for source in knowledge_data 
                    if not source.get('is_followup', False) and source.get('relevance_score', 0) >= 0.6
                ]
                
                follow_up_sources_used = [
                    source for source in knowledge_data 
                    if source.get('is_followup', False) and source.get('relevance_score', 0) >= 0.3
                ]
                
                # Sort by relevance
                primary_sources_used.sort(key=lambda x: x.get('relevance_score', 0), reverse=True)
                follow_up_sources_used.sort(key=lambda x: x.get('relevance_score', 0), reverse=True)
                
                source_text = "Sources used:"
                
                # Show top 3 primary sources
                for i, source in enumerate(primary_sources_used[:3], 1):
                    relevance = source.get('relevance_score', 0) * 100
                    source_name = source.get('source', 'Unknown')
                    source_text += f"\n{i}. {source_name} ({relevance:.0f}%)"
                
                # Show follow-up sources that provided implementation details
                if follow_up_sources_used:
                    source_text += f"\n\nImplementation details from:"
                    for i, source in enumerate(follow_up_sources_used, len(primary_sources_used) + 1):
                        relevance = source.get('relevance_score', 0) * 100
                        source_name = source.get('source', 'Unknown')
                        searched_for = source.get('searched_for', 'Unknown')
                        source_text += f"\n{i}. {source_name} ({relevance:.0f}%) - searched for: {searched_for}"
...
{source_text}"""
    ...
    async def get_system_info(self) -> Dict:
        ...
        try:
            ...
            categories_result = await self.mcp_sessions["classification"].call_tool(
                "get_team_categories", {"team": "atrs"}
            )
            categories_data = categories_result.content[0].text
            import json

            categories = (
                list(json.loads(categories_data).keys())
