...
def info():
    ...
    async def show_info():
        ...
        try:
            ...
            console.print(f"Knowledge Base Entries: {system_info.get('knowledge_base_entries', 0)}")
            console.print(f"Active Conversations: {system_info.get('active_conversations', 0)}")
...
async def run_sample_demo(config: MCPConfig):
    ...
    try:
        for i, issue in enumerate(sample_issues, 1):
            console.print(f"\n{'='*60}")
            console.print(f"[bold yellow]Demo Scenario #{i}: {issue['demo_type']}[/bold yellow]")
            console.print(f"{'='*60}")
            ...
            if result is not None:
                ...
                console.print(f"  ⎓ Done ({tool_count} tool {'use' if tool_count == 1 else 'uses'} · {token_str} · {elapsed:.1f}s)")
            if result is None:
...
async def display_analysis_results(result: dict):
    ...
    # Resolution Steps
    console.print(Panel(
        Markdown(recommendations["resolution_steps"]),
    ...
    console.print(Panel(
        f"[bold]Request ID:[/bold] {result['request_id']}\n"
        f"[bold]Confidence Score:[/bold] {metadata['confidence_score']:.1%}\n"
        f"[bold]Entities Processed:[/bold] {metadata['processed_entities']}\n"
        f"[bold]Sources Consulted:[/bold] {metadata['sources_consulted']}",
